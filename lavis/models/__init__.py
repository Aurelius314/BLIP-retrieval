...
def load_model_and_preprocess(name, model_type, is_eval=False, device="cpu", checkpoint=None):
    """
    ...
    Args:
        ...
        is_eval (bool): whether the model is in eval mode. Default: False.
        device (str): device to use. Default: "cpu".
        checkpoint (str): path to local checkpoint file. Default: None.
            If provided, will load weights from this local file instead of downloading.
            Note that expecting the checkpoint to have the same keys in state_dict as the model.
    ...
    # load checkpoint if provided (this will override the weights from config)
    if checkpoint is not None:
        model.load_checkpoint(checkpoint)
